...
def shuffle_sequences_add_motifs(sequenceFile, motifMatchFile, implantedFile):
	seqs = load_sequences(sequenceFile)
	motif_matches = load_motif_matches(motifMatchFile)
	fp=open(implantedFile, "w+")
	for key in seqs:
		origseq = seqs[key]
		...
		fp.write(">dinuc_shuffled_motifs_implanted_" + key + "\n")
		fp.write(newseq+"\n")
	fp.close()
