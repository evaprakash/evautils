...
def get_model(load_weights = True):
    ...
    if load_weights:
        pytorch_weights=torch.load('deepsea_variant_effects.pth')
        weights={}
        for k,v in pytorch_weights.items():
            key=re.match('((1\.)(.*))',k.decode("utf-8")).group(3)
            weights.update({key.encode("utf-8"):v})
        deepsea_cpu.load_state_dict(weights)
...
veff_model = get_model(load_weights=True)
predict_model = get_seqpred_model(load_weights=True)
